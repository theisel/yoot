...
async function main() {
  ...
  console.log(`✅ Merged ${packages.length} packages into ${OUTPUT_FILE}`);

  // Delete the individual index.md files
  await Promise.all(indexFiles.map((file) => fs.unlink(file)));
  console.log(`🗑️  Deleted ${indexFiles.length} index.md files`);
}

main().catch((err) => {
  console.error('❌ Error during documentation merge:', err);
  process.exit(1);
});
